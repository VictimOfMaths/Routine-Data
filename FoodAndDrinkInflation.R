...
CPIdata <-read.csv(temp) %>% 
  ...
  select(CDID, D7BT, D7D5:D7FR, D7CB) %>% 
  set_names("date", "Overall", "Bread & Cereals", "Meat", "Fish", 
            ...
            "Other General Services", "Tobacco") %>% 
  filter(nchar(date)>7) %>% 
...
agg_png("Outputs/CPIInflationxItem.png", units="in", width=9, height=7, res=500)
ggplot(CPIdata %>% filter(date>=as.Date("2021-01-01") &
  ...
  geom_text_repel(data=. %>% filter(date==max(date) & Product!="Overall"),
                  ...
                  xlim = c(as.Date("2024-10-10"), NA_Date_), show.legend=FALSE, segment.color = NA)+
  scale_x_date(name="", limits=c(NA_Date_, as.Date("2025-05-01")))+
...
agg_png("Outputs/CPIInflationxItemRel.png", units="in", width=9, height=7, res=500)
CPIdata %>% filter(date>=as.Date("2021-01-01") &
                     Product %in% c("Bread & Cereals", "Meat", "Fish", "Overall",
                                    "Milk, Cheese & Eggs", "Oils & Fats", "Fruit", 
                                    "Vegetables", "Sugar & Sweets","Tea & Coffee", 
                                    "Soft Drinks", "Spirits", "Wine", "Beer")) %>% 
  group_by(date) %>% 
  mutate(relchange=CPI_2021/CPI_2021[Product=="Overall"]) %>% 
  ungroup() %>% 
  ggplot(aes(x=date, y=relchange, colour=Product))+
  geom_hline(yintercept=1, colour="grey30")+
  geom_line(data=. %>% filter(Product!="Overall"), show.legend=FALSE)+
  geom_text_repel(data=. %>% filter(date==max(date) & Product!="Overall"),
                  aes(label = Product), size=3,
  ...
  scale_colour_manual(values=c("#EF7C12","#FC6882","#54BCD1", "#F4B95A", "#009F3F", "#8FDA04", 
                               "tomato","#AF6125", "#007BC3","#B25D91", "#EFC7E6", "#F4E3C7", "#C70E7B"))+
  theme_custom()+
  labs(title="The relative price of alcohol has fallen during the cost-of-living crisis",
       subtitle="Relative change in prices compared to overall UK CPI inflation for food and drink items since January 2021",
       caption="Data from ONS | Plot by @VictimOfMaths")+
  theme(panel.grid.major.y=element_line(colour="grey95"),
        axis.line.x=element_blank())
